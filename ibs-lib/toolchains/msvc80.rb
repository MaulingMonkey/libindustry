...
module Industry
	class MSVC80_Toolchain
		def export_solution( solution )
			File.open( solution.msvc80_solution_filename ,  File::CREAT | File::TRUNC | File::WRONLY ) do |file|
				file.puts ""
				file.puts "Microsoft Visual Studio Solution File, Format Version 9.00"
				file.puts "# Visual Studio 2005"

				sln_uuid      = "{#{solution.uuid}}".upcase
				...
				solution.projects.each do |name,project|
					proj_uuid     = "{#{project.uuid}}".upcase
					proj_filename = project.msvc80_project_filename
