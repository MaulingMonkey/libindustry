...
namespace industry {
	namespace pod {
		...
		struct interleaved_source {
			...
			template < BOOST_PP_ENUM_PARAMS( INDUSTRY_POD_INTERLEAVED_LIMIT , typename U ) >
			operator boost::tuples::tuple< BOOST_PP_ENUM_PARAMS( INDUSTRY_POD_INTERLEAVED_LIMIT , U ) >() const {
				return to_tuple();
			}
		};
